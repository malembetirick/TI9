...
public class TESTPlugin2_ implements PlugInFilter, DialogListener {
	...
	public void run(ImageProcessor ip) {
		...
		/* � compl�ter */
		float[] masque = this.masqueLoG(9, sigma);
		...
		conv.convolve(fpLaplacian,masque,9,9);
		ImagePlus newImg = new ImagePlus("LOG par 25 "+sigma, fpLaplacian);
        newImg.show();
		/*
		...
		if (seuillageZeroCross) {
			/* � compl�ter */
			ImagePlus im = new ImagePlus("seuillage laplacien"+sigma,this.laplacienZero(fpLaplacian,seuilZeroCross));
			im.show();
		}
	}
